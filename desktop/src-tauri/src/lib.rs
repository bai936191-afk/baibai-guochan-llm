...
fn window_state_path(app: &AppHandle) -> Option<PathBuf> {
    ...
    // %APPDATA%\cloud.xkxkbbk.baibai.desktop\.
...
fn setup_system_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let menu = MenuBuilder::new(app)
        .text(TRAY_SHOW_ID, "显示 白白国产大模型")
        .separator()
        .text(TRAY_QUIT_ID, "退出 白白国产大模型")
        .build()?;
...
pub fn run() {
    ...
    let builder = builder
        .menu(|app| {
            let about_item =
                MenuItemBuilder::with_id("nav_about", "关于 白白国产大模型").build(app)?;
            let settings_item = MenuItemBuilder::with_id("nav_settings", "设置...")
                .accelerator("CmdOrCtrl+,")
                .build(app)?;

            let app_submenu = SubmenuBuilder::new(app, "白白国产大模型")
                .item(&about_item)
                .separator()
                .item(&settings_item)
                .separator()
