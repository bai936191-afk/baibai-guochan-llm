...
import type { AgentTaskNotification, ToolProgressSnapshot } from '../../types/chat'
...
export const ToolCallBlock = memo(function ToolCallBlock({ toolName, input, result, compact = false, isPending = false, status, partialInput, progress }: Props) {
  ...
  const inputStatsSummary = liveStats ? formatContentStats(liveStats, t) : ''
  const liveProgressSummary = isPending && !result ? formatToolProgressSummary(progress, t) : ''
  const liveStatsSummary = liveProgressSummary || inputStatsSummary
  const liveOutputText = isPending && !result ? getLiveProgressOutput(progress) : ''
  ...
  const liveProgressOutput = useMemo(() => renderLiveProgressOutput(liveOutputText, progress, t), [liveOutputText, progress, t])
  ...
  const expandable = hasEditPreview || hasWritePreview || hasResultDetails || Boolean(isPending && (partialInput || liveOutputText))
  const showDetails = expandable && (expanded || Boolean(liveOutputText))
...
function isTerminalTool(toolName: string): boolean {
  return toolName === 'Bash' || toolName === 'PowerShell' || toolName === 'run_command'
}

function getLiveProgressOutput(progress?: ToolProgressSnapshot): string {
  return progress?.fullOutput || progress?.output || ''
}

function formatToolProgressSummary(
  progress?: ToolProgressSnapshot,
  t?: (key: TranslationKey, params?: Record<string, string | number>) => string,
): string {
  if (!progress) return ''
  const parts: string[] = []
  if (typeof progress.elapsedTimeSeconds === 'number') {
    parts.push(t?.('tool.runningSeconds', { seconds: progress.elapsedTimeSeconds }) ?? `${progress.elapsedTimeSeconds}s`)
  }
  if (typeof progress.totalLines === 'number' && progress.totalLines > 0) {
    parts.push(t?.('tool.liveLines', { count: progress.totalLines }) ?? `${progress.totalLines} lines`)
  }
  return parts.join(' · ')
}

function renderLiveProgressOutput(
  text: string,
  progress?: ToolProgressSnapshot,
  t?: (key: TranslationKey, params?: Record<string, string | number>) => string,
) {
  if (!text) return null
  const title = [
    t?.('tool.liveOutput') ?? 'Live Output',
    formatToolProgressSummary(progress, t),
  ].filter(Boolean).join(' · ')
  return (
    <TerminalChrome title={title}>
      <pre className="max-h-[360px] overflow-auto whitespace-pre-wrap break-words px-3 py-2.5 font-[var(--font-mono)] text-[11px] leading-[1.35] text-[var(--color-terminal-fg)]">
        {text}
      </pre>
    </TerminalChrome>
  )
...
function renderPreview(
  ...
  if (isTerminalTool(toolName) && typeof obj.command === 'string') {
    const title = typeof obj.description === 'string'
      ? obj.description
      : toolName === 'run_command'
        ? t?.('tool.terminal') ?? 'Terminal'
        : filePath
    return (
      <>
        <TerminalChrome title={title}>
...
function getVisibleResultText(
  ...
  if (toolName === 'Bash' || toolName === 'PowerShell' || toolName === 'Read' || toolName === 'Edit' || toolName === 'Write') return null
  return text
}
...
function getPendingSummary(
  ...
  if (isTerminalTool(toolName)) return t?.('tool.runningCommand') ?? 'Running command'
