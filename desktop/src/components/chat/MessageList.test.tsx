...
describe('MessageList nested tool calls', () => {
  ...
  it('shows message reset action alongside turn checkpoint undo', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Reset to this point' })).toBeTruthy()
  ...
  it('branches from completed transcript-backed chat messages using the original transcript id', async () => {
    vi.spyOn(sessionsApi, 'rewind').mockResolvedValue({
      target: {
        targetUserMessageId: 'local-user-1',
        userMessageIndex: 0,
        userMessageCount: 1,
      },
      conversation: {
        messagesRemoved: 2,
        removedMessageIds: ['local-user-1', 'local-assistant-1'],
      },
      code: {
        available: false,
        filesChanged: [],
    ...
    const userPrompt = 'start from here'
    ...
    expect(within(userActions as HTMLElement).queryByRole('button', { name: 'Fork a new conversation' })).toBeNull()
    fireEvent.click(within(userActions as HTMLElement).getByRole('button', { name: 'Reset to this point' }))

    await waitFor(() => {
      expect(sessionsApi.rewind).toHaveBeenCalledWith(ACTIVE_TAB, {
        targetUserMessageId: 'local-user-1',
        userMessageIndex: 0,
        expectedContent: userPrompt,
      })
    })
    ...
    expect(branchButtons[0]?.getAttribute('title')).toBe('Fork a new conversation')
  ...
  it('resets unanswered user messages and restores all referenced attachments', async () => {
    vi.spyOn(sessionsApi, 'rewind').mockResolvedValue({
      target: {
        targetUserMessageId: 'local-user-1',
        userMessageIndex: 0,
      ...
      conversation: {
        messagesRemoved: 1,
        removedMessageIds: ['local-user-1'],
      },
      code: {
        available: false,
        filesChanged: [],
    ...
    const prefixedModelContent = '@"C:\\repo\\extra.ts" analyze this'
    const fileAttachment = { type: 'file' as const, name: 'main.go', path: 'C:\\repo\\main.go' }
          ...
          messages: [
            {
              ...
              type: 'user_text',
              content: userPrompt,
              modelContent: prefixedModelContent,
    ...
    const userActions = (await screen.findByRole('button', { name: 'Copy prompt' })).closest('[data-message-actions]')
    expect(userActions).toBeTruthy()
    fireEvent.click(within(userActions as HTMLElement).getByRole('button', { name: 'Reset to this point' }))
    ...
    await waitFor(() => {
      expect(sessionsApi.rewind).toHaveBeenCalledWith(ACTIVE_TAB, {
        targetUserMessageId: 'local-user-1',
        userMessageIndex: 0,
        expectedContent: prefixedModelContent,
      })
    })
    ...
    expect(queueComposerPrefill).toHaveBeenCalledWith(ACTIVE_TAB, {
      ...
      attachments: [
        fileAttachment,
        imageAttachment,
        { type: 'file', name: 'extra.ts', path: 'C:\\repo\\extra.ts' },
      ],
      reuseEmptySessionOnSubmit: true,
    })
  })

  it('locally rewinds interrupted pending user messages when the server has no transcript target', async () => {
    vi.spyOn(sessionsApi, 'rewind').mockRejectedValue(new Error('Invalid rewind target'))
    const rewindConversationLocally = vi.fn()
    const stopGeneration = vi.fn()
    const imageAttachment = {
      type: 'image' as const,
      name: 'screen.png',
          ...
          messages: [
            {
              ...
              type: 'user_text',
              content: 'hello',
              modelContent: '@"C:\\repo\\context.md" hello',
    ...
    await waitFor(() => {
      expect(sessionsApi.rewind).toHaveBeenCalledWith(ACTIVE_TAB, {
        targetUserMessageId: 'pending-user-1',
        userMessageIndex: 0,
        expectedContent: '@"C:\\repo\\context.md" hello',
      })
    })
    expect(stopGeneration).toHaveBeenCalledWith(ACTIVE_TAB)
    expect(rewindConversationLocally).toHaveBeenCalledWith(ACTIVE_TAB, {
      messageId: 'pending-user-1',
      text: 'hello',
      attachments: [
        imageAttachment,
        { type: 'file', name: 'context.md', path: 'C:\\repo\\context.md' },
      ],
    })
  })
