...
describe('ChatInput file mentions', () => {
  ...
  it('restores rewind prefill text with file path and image attachments', async () => {
    render(<ChatInput compact />)

    act(() => {
      useChatStore.getState().queueComposerPrefill(sessionId, {
        text: 'revise this turn',
        attachments: [
          {
            type: 'file',
            name: 'main.go',
    ...
    await waitFor(() => {
      expect(screen.getByRole('textbox')).toHaveValue('revise this turn')
  ...
  it('reuses a rewound empty session instead of creating a replacement session', async () => {
    useSessionStore.setState({
      sessions: [{
        id: sessionId,
        title: 'Project',
    ...
    await waitFor(() => {
      expect(input).toHaveValue('edit this prompt again')
    })
    expect(screen.queryByRole('button', { name: /Select branch:/ })).not.toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(mocks.wsSend).toHaveBeenCalledWith(sessionId, {
        type: 'user_message',
        content: 'edit this prompt again',
        attachments: [],
      })
    })
    expect(mocks.create).not.toHaveBeenCalled()
    expect(mocks.delete).not.toHaveBeenCalled()
    expect(useChatStore.getState().sessions[sessionId]?.reuseEmptySessionOnNextSubmit).toBe(false)
  })
