import { Check, Copy, CornerUpLeft, GitFork } from 'lucide-react'
...
export type MessageResetAction = {
  label: string
  loading?: boolean
  onReset: () => void
}
...
type Props = {
  ...
  resetAction?: MessageResetAction
  align?: 'start' | 'end'
  timestamp?: number
}
...
export function MessageActionBar({
  ...
  if (!hasCopy && !branchAction && !resetAction) return null
    ...
    >
      <div className="flex min-h-7 items-center gap-1.5">
        {resetAction ? (
          ...
          >
            <CornerUpLeft size={14} strokeWidth={2.1} aria-hidden="true" />
          </button>
        ) : null}
        {hasCopy ? (
          <CopyButton
