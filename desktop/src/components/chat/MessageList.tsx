...
function extractFileReferenceAttachments(text: string | undefined): UIAttachment[] {
  if (!text) return []
  ...
  while (true) {
    ...
    attachments.push({
      type: 'file',
      name: normalized.split('/').filter(Boolean).pop() || path,
      path,
    })
...
function mergeRewindAttachments(
  messageAttachments: Extract<UIMessage, { type: 'user_text' }>['attachments'],
  modelContent: string | undefined,
): Extract<UIMessage, { type: 'user_text' }>['attachments'] {
  const modelAttachments = extractFileReferenceAttachments(modelContent)
  if (!messageAttachments?.length) {
    return modelAttachments.length > 0 ? modelAttachments : messageAttachments
  }
  if (modelAttachments.length === 0) return messageAttachments
...
export function getUserMessageRewindTargets(messages: UIMessage[]): RewindTurnTarget[] {
  ...
  for (const message of messages) {
    ...
    targets.push({
      ...
      attachments: mergeRewindAttachments(message.attachments, message.modelContent),
    })
  }
...
function getMessageContentWeight(message: UIMessage): number {
  switch (message.type) {
    ...
    case 'tool_use':
      return getShallowStringWeight(message.input) + (message.partialInput?.length ?? 0) + (message.progress?.fullOutput?.length ?? message.progress?.output?.length ?? 0)
    case 'tool_result':
...
function getMessageMetricSignature(message: UIMessage): string {
  switch (message.type) {
    ...
    case 'tool_use':
      return `${message.type}:${message.toolName}:${message.toolUseId}:${message.partialInput?.length ?? 0}:${message.progress?.fullOutput?.length ?? message.progress?.output?.length ?? 0}:${message.isPending ? 1 : 0}:${message.status ?? ''}`
    case 'tool_result':
...
export function MessageList({ sessionId, compact = false }: MessageListProps = {}) {
  ...
  const rewindConversationLocally = useChatStore((s) => s.rewindConversationLocally)
  ...
  const userMessageRewindTargets = useMemo(
    () => getUserMessageRewindTargets(messages),
    [messages],
  )
  ...
  const handleUndoCurrentTurn = useCallback(async () => {
    ...
    try {
      ...
      queueComposerPrefill(resolvedSessionId, {
        ...
        reuseEmptySessionOnSubmit: true,
      })
  ...
  const handleResetToMessage = useCallback(async (target: RewindTurnTarget) => {
    ...
    try {
      ...
      rewindConversationLocally(resolvedSessionId, {
      ...
      addToast({
        type: 'success',
        message: t('chat.rewindSuccessConversationOnly', { count: 1 }),
      })
    } finally {
      setRewindingTurnId(null)
    }
  ...
  const resetActionByMessageId = useMemo(() => {
    if (isMemberSession || userMessageRewindTargets.length === 0) {
      return new Map<string, { label: string; loading: boolean; onReset: () => void }>()
    }
    const result = new Map<string, { label: string; loading: boolean; onReset: () => void }>()
    const label = t('chat.resetToPoint')
    for (const target of userMessageRewindTargets) {
      result.set(target.messageId, {
        label,
        loading: rewindingTurnId === target.messageId,
        onReset: () => { void handleResetToMessage(target) },
      })
    }
    return result
  }, [handleResetToMessage, isMemberSession, rewindingTurnId, t, userMessageRewindTargets])
      ...
      <>
        {item.kind === 'tool_group' ? (
          ...
          <MessageBlock
            ...
            resetAction={resetActionByMessageId.get(item.message.id)}
