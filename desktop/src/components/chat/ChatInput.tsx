...
function prefillAttachmentToComposerAttachment(
  attachment: UIAttachment,
  nonce: number,
  index: number,
): Attachment {
  return {
    id: `composer-prefill-${nonce}-${index}`,
...
export function ChatInput({ variant = 'default', compact = false }: ChatInputProps) {
  ...
  const {
    ...
    clearReuseEmptySessionOnNextSubmit,
  ...
  const reuseEmptySessionOnNextSubmit = sessionState?.reuseEmptySessionOnNextSubmit ?? false
  ...
  const showLaunchControls = !isMemberSession && messageCount === 0 && !reuseEmptySessionOnNextSubmit
  ...
  useEffect(() => {
    ...
    const nextAttachments = (composerPrefill.attachments ?? [])
      .map((attachment, index) => prefillAttachmentToComposerAttachment(attachment, composerPrefill.nonce, index))
  ...
  const handleSubmit = async () => {
    ...
    clearReuseEmptySessionOnNextSubmit(activeTabId!)
    if (targetSessionId !== activeTabId) useChatStore.getState().clearComposerDraft(targetSessionId)
    if (targetSessionId !== activeTabId) clearReuseEmptySessionOnNextSubmit(targetSessionId)
    if (!isMemberSession) {
