...
describe('ModelSelector', () => {
  ...
  it('shows model capabilities from provider metadata', async () => {
    const models: ModelInfo[] = [
      {
        id: 'minimax-m3',
        name: 'Minimax M3',
    ...
    render(<ModelSelector value="minimax-m3" onChange={vi.fn()} />)

    await clickByRole(/Minimax M3/i)

    expect(screen.getByText('Reasoning Image')).toBeTruthy()
  })
  ...
  it('refreshes provider metadata when the runtime dropdown opens', async () => {
    ...
    render(<ModelSelector runtimeKey="session-refresh-models" />)
