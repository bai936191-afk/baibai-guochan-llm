...
function getModelCapabilityDescription(
  model: Pick<ModelInfo, 'capabilities'>,
  labels: ModelCapabilityLabels,
): string {
  const normalized = new Set((model.capabilities ?? []).map((capability) => capability.trim().toLowerCase()))
  const capabilities: string[] = []
  if (normalized.has('reasoning') || normalized.has('thinking') || normalized.has('text')) {
    capabilities.push(labels.reasoning)
  }
  if (normalized.has('image') || normalized.has('vision')) {
    capabilities.push(labels.image)
  }
...
function buildProviderModels(
  ...
  return [...byId.values()].map((entry) => ({
    id: entry.id,
    name: entry.id,
    description: getModelCapabilityDescription({}, capabilityLabels),
    context: '',
  }))
}

function buildCompleteProviderModels(
  provider: SavedProvider,
  labels: Record<'main' | 'haiku' | 'sonnet' | 'opus', string>,
  capabilityLabels: ModelCapabilityLabels,
): ModelInfo[] {
  if (!provider.availableModels || provider.availableModels.length === 0) {
    return buildProviderModels(provider, labels, capabilityLabels)
  }

  const roleEntries: Array<{ id: string }> = [
    { id: provider.models.main.trim() },
    { id: provider.models.haiku.trim() },
    { id: provider.models.sonnet.trim() },
    { id: provider.models.opus.trim() },
  ]
  const byId = new Map<string, ModelInfo>()
  const addModel = (id: string, contextWindow?: number, capabilities?: string[]) => {
    ...
    byId.set(trimmed, {
      id: trimmed,
      name: trimmed,
      description: getModelCapabilityDescription({ capabilities }, capabilityLabels),
      context: '',
  ...
  for (const model of provider.availableModels ?? []) {
    addModel(model.id, model.contextWindow, model.capabilities)
  }
...
function buildProviderChoices(
  ...
  for (const provider of providers) {
    choices.push({
      ...
      models: buildCompleteProviderModels(provider, labels, capabilityLabels),
    })
  }
...
export function ModelSelector({
  ...
  useEffect(() => {
    if (!open || !isRuntimeScoped || providersLoading) return
    void fetchProviders()
  }, [fetchProviders, isRuntimeScoped, open, providersLoading])
  ...
  const capabilityLabels = useMemo(
    () => ({
      reasoning: t('model.capability.reasoning'),
      image: t('model.capability.image'),
    }),
    [t],
  )
  ...
  const providerChoices = useMemo(
    ...
    [activeId, capabilityLabels, providers, roleLabels],
  )
  ...
  const buttonProviderLabel = isRuntimeScoped
    ? selectedProviderChoice?.providerName ?? activeProviderName ?? ''
    : null
        ...
        {isRuntimeScoped ? (
          ...
          <div className="space-y-1">
            {availableModels.map((model) => {
              ...
              const description = getModelCapabilityDescription(model, capabilityLabels) || model.description
              return (
                <button
                  key={model.id}
                  onClick={() => {
