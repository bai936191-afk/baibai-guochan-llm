...
describe('Settings > Providers tab', () => {
  ...
  it('keeps the built-in Local API out of the add-provider presets', () => {
    providerStoreState.presets = [
      {
        id: 'local-openai',
        name: 'Local API',
    ...
    expect(within(dialog).queryByRole('button', { name: 'Local API' })).not.toBeInTheDocument()
    expect(within(dialog).getByRole('button', { name: 'DeepSeek' })).toBeInTheDocument()
    expect(within(dialog).getByDisplayValue('https://api.deepseek.com/anthropic')).toBeInTheDocument()
    expect(within(dialog).getByText('Model Mapping')).toBeInTheDocument()
  })

  it('shows a single model field when editing the built-in Local API provider', () => {
    providerStoreState.providers = [
      {
        id: 'provider-local',
        name: 'Local API',
    ...
    providerStoreState.providerOrder = ['provider-local']
    providerStoreState.presets = [
      {
        id: 'deepseek',
        name: 'DeepSeek',
    ...
    expect(within(dialog).queryByText('Enable Tool Search')).not.toBeInTheDocument()
    expect(within(dialog).queryByText('Model context')).not.toBeInTheDocument()
