...
describe('chatStore history mapping', () => {
  ...
  it('keeps persisted file block paths when restoring user history', () => {
    const mapped = mapHistoryMessagesToUiMessages([
      {
        id: 'file-user-1',
        type: 'user',
        ...
        content: [
          ...
          { type: 'file', name: 'main.go', path: 'C:\\repo\\main.go' },
        ],
      },
    ])
  ...
  it('updates a running terminal tool with live progress output', () => {
    useChatStore.setState({
      sessions: {
        [TEST_SESSION_ID]: makeSession(),
      },
