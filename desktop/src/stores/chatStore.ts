...
export type PerSessionState = {
  ...
  reuseEmptySessionOnNextSubmit?: boolean
...
const DEFAULT_SESSION_STATE: PerSessionState = {
  ...
  reuseEmptySessionOnNextSubmit: false,
...
type ChatStore = {
  ...
  rewindConversationLocally: (
    sessionId: string,
    target: { messageId: string; text: string; attachments?: UIAttachment[] },
  ) => void
  loadHistory: (sessionId: string) => Promise<void>
  reloadHistory: (sessionId: string) => Promise<void>
  queueComposerPrefill: (
    sessionId: string,
    prefill: { text: string; attachments?: UIAttachment[]; mode?: ComposerPrefillMode; reuseEmptySessionOnSubmit?: boolean },
  ) => void
  clearComposerPrefill: (sessionId: string, nonce?: number) => void
  clearReuseEmptySessionOnNextSubmit: (sessionId: string) => void
...
function mergeToolProgress(
  existing: ToolProgressSnapshot | undefined,
  update: ToolProgressSnapshot,
): ToolProgressSnapshot {
  return {
    ...existing,
    ...update,
    output: update.output ?? existing?.output,
    fullOutput: update.fullOutput ?? existing?.fullOutput,
  }
}

function clearToolUsePending(messages: UIMessage[], toolUseId: string): UIMessage[] {
  let changed = false
  const next = messages.map((message) => {
    if (message.type !== 'tool_use' || message.toolUseId !== toolUseId) return message
    if (!message.isPending && !message.progress) return message
    changed = true
    return {
      ...message,
      isPending: false,
...
export const useChatStore = create<ChatStore>((set, get) => ({
    ...
    set((s) => ({
      sessions: {
        ...
        [sessionId]: {
          ...
          reuseEmptySessionOnNextSubmit: existing?.reuseEmptySessionOnNextSubmit ?? false,
        },
      },
    }))
  ...
  rewindConversationLocally: (sessionId, target) => {
    ...
    set((s) => {
      ...
      if (!session) return s
      if (session.elapsedTimer) clearInterval(session.elapsedTimer)
      const targetIndex = session.messages.findIndex((message) => message.id === target.messageId)
      ...
      return {
        sessions: {
          ...
          [sessionId]: {
            ...
            reuseEmptySessionOnNextSubmit: true,
          },
        },
      }
    })
  ...
  queueComposerPrefill: (sessionId, prefill) => {
    set((state) => ({
      sessions: updateSessionIn(state.sessions, sessionId, () => ({
        ...
        ...(prefill.reuseEmptySessionOnSubmit ? { reuseEmptySessionOnNextSubmit: true } : {}),
      })),
    }))
  },
  ...
  clearReuseEmptySessionOnNextSubmit: (sessionId) => {
    set((state) => ({
      sessions: updateSessionIn(state.sessions, sessionId, () => ({
        reuseEmptySessionOnNextSubmit: false,
      })),
  ...
  handleServerMessage: (sessionId, msg) => {
    ...
    switch (msg.type) {
      ...
      case 'tool_progress': {
        ...
        const parentToolUseId = msg.parentToolUseId ?? getPendingToolParentUseId(sessionId, toolUseId)
        rememberPendingToolParentUseId(sessionId, toolUseId, parentToolUseId)
        const progress: ToolProgressSnapshot = {
          ...(msg.toolName ? { toolName: msg.toolName } : {}),
          ...(parentToolUseId ? { parentToolUseId } : {}),
          ...(msg.progressId ? { progressId: msg.progressId } : {}),
          ...(msg.output !== undefined ? { output: msg.output } : {}),
          ...(msg.fullOutput !== undefined ? { fullOutput: msg.fullOutput } : {}),
          ...(msg.elapsedTimeSeconds !== undefined ? { elapsedTimeSeconds: msg.elapsedTimeSeconds } : {}),
          ...(msg.totalLines !== undefined ? { totalLines: msg.totalLines } : {}),
          ...(msg.totalBytes !== undefined ? { totalBytes: msg.totalBytes } : {}),
        ...
        update((s) => ({
          messages: upsertToolUseMessage(s.messages, toolUseId, (existing) => ({
            id: existing?.id ?? nextId(),
            type: 'tool_use',
            toolName: existing?.toolName ?? msg.toolName ?? s.activeToolName ?? 'run_command',
            toolUseId,
            input: existing?.input ?? {},
            timestamp: existing?.timestamp ?? Date.now(),
            ...
            isPending: true,
            partialInput: existing?.partialInput,
            progress: mergeToolProgress(existing?.progress, progress),
          })),
          chatState: 'tool_executing',
          activeThinkingId: null,
          apiRetry: null,
          streamingFallback: null,
          statusVerb: msg.elapsedTimeSeconds !== undefined
            ? `${msg.toolName ?? 'tool'} ${msg.elapsedTimeSeconds}s`
            : s.statusVerb,
        }))
        useTabStore.getState().updateTabStatus(sessionId, 'running')
        break
      ...
      case 'tool_result': {
        ...
        update((s) => {
          let messages: UIMessage[] = [...clearToolUsePending(s.messages, msg.toolUseId), {
...
function normalizeHistoryFileAttachment(block: UserHistoryBlock): UIAttachment {
  const path = block.path ?? block.file_path ?? block.filePath
  const name = block.name ?? (path ? getReferenceName(path) : 'file')
  return {
    type: 'file',
    name,
    path,
    isDirectory: block.isDirectory ?? block.is_directory,
  }
}
  ...
  for (const msg of messages) {
    ...
    if ((msg.type === 'user' || msg.type === 'tool_result') && Array.isArray(msg.content)) {
      ...
      for (const block of msg.content as UserHistoryBlock[]) {
        ...
        else if (block.type === 'file') attachments.push(normalizeHistoryFileAttachment(block))
