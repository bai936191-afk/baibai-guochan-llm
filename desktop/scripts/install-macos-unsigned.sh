...
#
# Installs the unsigned macOS build of 白白国产大模型 from a DMG that sits
...
#   bash install-macos-unsigned.sh /path/to/白白国产大模型-0.4.0-mac-arm64.dmg

APP_NAME="白白国产大模型.app"
...
find_dmg() {
  ...
  shopt -s nullglob
  local matches=("${dir}"/白白国产大模型-*-mac-*.dmg)
  shopt -u nullglob
...
main() {
  ...
  if [ -z "$dmg" ]; then
    if ! dmg="$(find_dmg "$base_dir")"; then
      echo "No 白白国产大模型 macOS DMG found next to this script."
      echo "Download the DMG into the same folder as this script, then run it again."
      echo "Usage: bash install-macos-unsigned.sh /path/to/白白国产大模型-0.4.0-mac-arm64.dmg"
      exit 1
    fi
  fi
  ...
  osascript -e 'quit app "白白国产大模型"' >/dev/null 2>&1 || true

  if [ -d "$APP_PATH" ]; then
    local backup
    backup="${HOME}/.Trash/白白国产大模型.$(date +%Y%m%d%H%M%S).app"
