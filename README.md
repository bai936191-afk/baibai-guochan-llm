...
| Linux x64 | `Baibai-Guochan-LLM-0.4.5-linux-x86_64.AppImage` 或 `Baibai-Guochan-LLM-0.4.5-linux-amd64.deb` |
| Linux ARM64 | `Baibai-Guochan-LLM-0.4.5-linux-arm64.AppImage` 或 `Baibai-Guochan-LLM-0.4.5-linux-arm64.deb` |
...
1. 下载 `Baibai-Guochan-LLM-0.4.5-win-x64.exe`。
2. 双击运行安装程序。
3. 选择安装路径，完成安装。
4. 打开桌面快捷方式，输入密钥。
...
```bash
chmod +x Baibai-Guochan-LLM-0.4.5-linux-x86_64.AppImage
./Baibai-Guochan-LLM-0.4.5-linux-x86_64.AppImage
```

Debian / Ubuntu：

```bash
sudo apt install ./Baibai-Guochan-LLM-0.4.5-linux-amd64.deb
```
