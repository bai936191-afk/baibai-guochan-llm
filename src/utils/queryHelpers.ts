...
const TOOL_PROGRESS_THROTTLE_MS = 1000
  ...
  switch (message.type) {
    ...
    case 'progress':
      if (
        ...
        // Desktop SDK consumers need this locally as well, otherwise long
        // run_command/Bash/PowerShell scripts look frozen until tool_result.
        const trackingKey = message.parentToolUseID
        const now = Date.now()
        const lastSent = toolProgressLastSentTime.get(trackingKey) || 0
        const timeSinceLastSent = now - lastSent

        // Send if at least a second has passed since last update.
        if (timeSinceLastSent >= TOOL_PROGRESS_THROTTLE_MS) {
          ...
          yield {
            type: 'tool_progress',
            tool_use_id: message.parentToolUseID || message.toolUseID,
            tool_name:
              message.data.type === 'bash_progress' ? 'Bash' : 'PowerShell',
            parent_tool_use_id: null,
            progress_id: message.toolUseID,
            output: message.data.output,
            full_output: message.data.fullOutput,
            elapsed_time_seconds: message.data.elapsedTimeSeconds,
            total_lines: message.data.totalLines,
            total_bytes: message.data.totalBytes,
            timeout_ms: message.data.timeoutMs,
