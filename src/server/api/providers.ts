...
export async function handleProvidersApi(
  ...
  try {
    ...
    if (!id) {
      if (req.method === 'GET') {
        await providerService.refreshActiveProviderModels()
...
async function handleFetchModels(req: Request): Promise<Response> {
  const body = await parseJsonBody(req)
  try {
    const input = TestProviderSchema.omit({ modelId: true }).parse(body)
    const result = await providerService.fetchModels(input)
    return Response.json(result)
  } catch (err) {
    if (err instanceof z.ZodError) throw ApiError.badRequest(err.issues.map((i) => i.message).join('; '))
    throw err
