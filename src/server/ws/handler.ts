...
function toToolProgressServerMessages(cliMsg: any, streamState: SessionStreamState): ServerMessage[] {
  const rawToolUseId = optionalString(cliMsg.tool_use_id)
  const rawParentToolUseId = cliParentToolUseId(cliMsg)
  const parentToolUseId =
    rawParentToolUseId && rawParentToolUseId !== rawToolUseId
      ? rawParentToolUseId
      : undefined
  const toolUseId = rawToolUseId || parentToolUseId
  if (!toolUseId) return []

  rememberToolParentUseId(streamState, toolUseId, parentToolUseId)

  const toolName = optionalString(cliMsg.tool_name)
  const output = optionalString(cliMsg.output)
  const fullOutput = optionalString(cliMsg.full_output)
  const elapsedTimeSeconds = optionalFiniteNumber(cliMsg.elapsed_time_seconds)
  const progressMessage: ServerMessage = {
    type: 'tool_progress',
    toolUseId,
    ...(toolName ? { toolName } : {}),
    ...(parentToolUseId ? { parentToolUseId } : {}),
    ...(optionalString(cliMsg.progress_id) ? { progressId: optionalString(cliMsg.progress_id) } : {}),
    ...(output !== undefined ? { output } : {}),
    ...(fullOutput !== undefined ? { fullOutput } : {}),
    ...(elapsedTimeSeconds !== undefined ? { elapsedTimeSeconds } : {}),
    ...(optionalFiniteNumber(cliMsg.total_lines) !== undefined ? { totalLines: optionalFiniteNumber(cliMsg.total_lines) } : {}),
    ...(optionalFiniteNumber(cliMsg.total_bytes) !== undefined ? { totalBytes: optionalFiniteNumber(cliMsg.total_bytes) } : {}),
    ...(optionalFiniteNumber(cliMsg.timeout_ms) !== undefined ? { timeoutMs: optionalFiniteNumber(cliMsg.timeout_ms) } : {}),
    ...(optionalString(cliMsg.task_id) ? { taskId: optionalString(cliMsg.task_id) } : {}),
  }

  const runningLabel = [
    toolName,
    elapsedTimeSeconds !== undefined ? `${elapsedTimeSeconds}s` : undefined,
  ].filter(Boolean).join(' ')
  ...
  return [
    progressMessage,
    {
      ...
      ...(runningLabel ? { verb: runningLabel } : {}),
    },
  ]
}
...
export function translateCliMessage(cliMsg: any, sessionId: string): ServerMessage[] {
  ...
  switch (cliMsg.type) {
    ...
    case 'tool_progress':
      return toToolProgressServerMessages(cliMsg, streamState)
