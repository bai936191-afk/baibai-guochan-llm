...
export class ConversationService {
  ...
  async startSession(
    ...
    const requestedWorkDir = this.expandHomeWorkDir(workDir)

    if (!fs.existsSync(requestedWorkDir) || !fs.statSync(requestedWorkDir).isDirectory()) {
      throw new ConversationStartupError(
        `Working directory does not exist or is not a directory: ${requestedWorkDir}`,
        'WORKDIR_INVALID',
      )
    }

    if (shouldReplacePlaceholder) {
      await sessionService.clearSessionTranscript(sessionId, requestedWorkDir)
    }

    let launchWorkDir = requestedWorkDir
    let launchRepository = launchInfo?.repository
    if (shouldCreateWorktree && launchRepository?.worktree) {
      launchWorkDir = this.expandHomeWorkDir(
        launchRepository.requestedWorkDir || launchRepository.repoRoot || requestedWorkDir,
      )
    ...
    const earlyExitCode = await Promise.race([
      proc.exited,
      this.waitForStartupReadyOrTimeout(session, STARTUP_GRACE_MS),
    ])
    ...
    if (startupExitCode !== null) {
      ...
      if (this.sessions.get(sessionId) !== session) {
        throw new ConversationStartupError(
          `CLI startup was cancelled before completion for ${sessionId}.`,
          'SESSION_DELETED',
          true,
        )
      }
      ...
      const startupRetryAttempt = options?.startupRetryAttempt ?? 0
      if (
        startupExitCode === 143 &&
        !capturedOutput &&
        session.sdkMessages.length === 0 &&
        startupRetryAttempt < 1
      ) {
        this.sessions.delete(sessionId)
        void diagnosticsService.recordEvent({
          type: 'cli_start_retry',
          severity: 'info',
          sessionId,
          summary: 'CLI exited during startup with code 143 and no output; retrying once.',
          details: {
            exitCode: startupExitCode,
            retryAttempt: startupRetryAttempt + 1,
            workDir: launchWorkDir,
            permissionMode: options?.permissionMode || 'default',
            providerId: options?.providerId ?? null,
            model: options?.model ?? null,
        ...
        return this.startSession(sessionId, requestedWorkDir, sdkUrl, {
          ...options,
          startupRetryAttempt: startupRetryAttempt + 1,
        })
      }
  ...
  private expandHomeWorkDir(workDir: string): string {
  ...
  private waitForStartupReadyOrTimeout(
    session: SessionProcess,
    timeoutMs: number,
  ): Promise<null> {
    if (session.initMessage) return Promise.resolve(null)

    return new Promise<null>((resolve) => {
      const startedAt = Date.now()
      const timer = setInterval(() => {
        if (session.initMessage || Date.now() - startedAt >= timeoutMs) {
          clearInterval(timer)
          resolve(null)
        }
  ...
  private extractStartupDetail(message: any): string {
    ...
    if (message?.type === 'result' && message.is_error) {
      ...
      if (typeof message.subtype === 'string') {
        attributes.push(`subtype=${message.subtype}`)
      }
      if (typeof message.stop_reason === 'string') {
        attributes.push(`stop_reason=${message.stop_reason}`)
      } else if (message.stop_reason === null) {
        attributes.push('stop_reason=null')
      }
      ...
      if (usage && typeof usage === 'object') {
        ...
        const tokenParts = [
          ...
          ['cache_read', usageRecord.cache_read_input_tokens],
          ['cache_create', usageRecord.cache_creation_input_tokens],
          ['output', usageRecord.output_tokens],
        ]
          .filter(([, value]) => typeof value === 'number')
          .map(([name, value]) => `${name}=${value}`)
        if (tokenParts.length > 0) {
          attributes.push(`usage(${tokenParts.join(', ')})`)
        }
      }

      return attributes.length > 0
        ? `SDK result error (${attributes.join(', ')})`
        : 'SDK result error'
    }
        ...
        details: {
          ...
          message:
            typeof message.message === 'string'
              ? this.redactProcessOutput(message.message)
              : undefined,
