...
function expandHomeWorkDir(workDir: string): string {
...
export type SessionExport = {
  ...
  metadata: {
    ...
    transcriptMetadata: TranscriptMetadataSnapshot | null
    transcriptUsage: TranscriptUsageSnapshot | null
    transcriptContextEstimate: TranscriptContextEstimate | null
  }
}
...
export class SessionService {
  ...
  async exportSession(sessionId: string): Promise<SessionExport> {
    ...
    const [
      ...
      this.readJsonlFile(found.filePath),
      this.getSessionTaskNotifications(sessionId),
      this.getSessionFileHistorySnapshots(sessionId),
      this.getSessionLaunchInfo(sessionId).catch(() => null),
      this.getTranscriptMetadata(sessionId).catch(() => null),
      this.getTranscriptUsage(sessionId).catch(() => null),
      this.getTranscriptContextEstimate(sessionId).catch(() => null),
    ])
  ...
  async createSession(
    ...
    const resolvedWorkDir = workDir?.trim()
      ? expandHomeWorkDir(workDir)
      : os.homedir()
