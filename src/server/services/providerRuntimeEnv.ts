...
const LOCAL_API_DEFAULT_ENV = {
  CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC: '1',
} satisfies Record<string, string>
const LOCAL_API_MODEL_CONTEXT_WINDOWS = {
  'glm-5.2': 1000000,
...
function normalizeContextWindow(value: unknown): number | undefined {
  ...
  if (parsed < 16_000 || parsed > 10_000_000) return undefined
  return parsed
}
...
function normalizeAvailableModels(value: unknown): SavedProvider['availableModels'] | undefined {
  if (!Array.isArray(value)) return undefined
  const seen = new Set<string>()
  const models: NonNullable<SavedProvider['availableModels']> = []
...
function shouldApplyModel1mSupport(provider: SavedProvider): boolean {
  return provider.presetId !== LOCAL_API_PRESET_ID
}
...
export function normalizeSavedProvider(provider: SavedProvider): SavedProvider {
  ...
  const availableModels = normalizeAvailableModels(rawProvider.availableModels)
  return {
    ...rest,
...
export function getPresetDefaultEnv(presetId: string): Record<string, string> {
  if (presetId === LOCAL_API_PRESET_ID) return LOCAL_API_DEFAULT_ENV
...
export function getPresetAuthStrategy(presetId: string): ProviderAuthStrategy {
  if (presetId === LOCAL_API_PRESET_ID) return 'api_key'
...
function getPresetModelContextWindows(presetId: string): Record<string, number> {
  if (presetId === LOCAL_API_PRESET_ID) return LOCAL_API_MODEL_CONTEXT_WINDOWS
...
export function buildProviderManagedEnv(
  ...
  const runtimeModels = applyModel1mSupportMapping(
    models,
    shouldApplyModel1mSupport(provider) ? provider.model1mSupport : undefined,
  )
