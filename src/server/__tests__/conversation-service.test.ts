...
describe('ConversationService', () => {
  ...
  test('extractStartupDetail preserves SDK result error diagnostics without a text payload', () => {
    const service = new ConversationService() as any
    const detail = service.extractStartupDetail({
      type: 'result',
    ...
    expect(detail).toContain('SDK result error')
    expect(detail).toContain('subtype=error_during_execution')
    expect(detail).toContain('stop_reason=tool_use')
    expect(detail).toContain('input=1436')
    expect(detail).toContain('cache_read=5272')
    expect(detail).toContain('output=18')
  })
})
