...
describe('ProviderService', () => {
  ...
  describe('addProvider', () => {
    ...
    test('local-openai providers ignore legacy model1mSupport and use explicit context windows', async () => {
      const svc = new ProviderService()
      const provider = await svc.addProvider(sampleInput({
        presetId: 'local-openai',
        name: 'Local API',
        baseUrl: 'https://ai.xkxkbbk.cloud',
      ...
      const runtimeEnv = await svc.getProviderRuntimeEnv(provider.id)
      expect(runtimeEnv.ANTHROPIC_MODEL).toBe('glm-5.2')
      expect(runtimeEnv.ANTHROPIC_DEFAULT_HAIKU_MODEL).toBe('glm-5.2')
      expect(runtimeEnv.ANTHROPIC_DEFAULT_SONNET_MODEL).toBe('glm-5.2')
      expect(runtimeEnv.ANTHROPIC_DEFAULT_OPUS_MODEL).toBe('glm-5.2')
      expect(runtimeEnv.CLAUDE_CODE_MODEL_CONTEXT_WINDOWS).toBe('{"glm-5.2":1000000}')
    })
  ...
  describe('fetchModels', () => {
    test('reads per-model context windows from OpenAI and Comate-style fields', async () => {
      const originalFetch = globalThis.fetch
      globalThis.fetch = mock(async () => {
        return new Response(JSON.stringify({
          data: [
            { id: 'glm-5.2', maxContextWindow: 1000 },
            { id: 'glm-5.1', max_input_tokens: '200000' },
            { id: 'deepseek-v4-flash', context_window: 128000 },
            { id: 'kimi-k2.6', metadata: { maxContextWindow: '1m' } },
            { id: 'minimax-m3', maxContextWindow: 1000, supportThinking: true, supportImage: true, input_modalities: ['text', 'image'] },
            { id: 'text-only', context_window: 128000, input_modalities: ['text'] },
          ],
        }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
      ...
      try {
        ...
        expect(result.models).toEqual([
          ...
          { id: 'kimi-k2.6', contextWindow: 1_000_000 },
          { id: 'minimax-m3', contextWindow: 1_000_000, capabilities: ['reasoning', 'image'] },
          { id: 'text-only', contextWindow: 128_000 },
        ])
      } finally {
        globalThis.fetch = originalFetch
      }
    ...
    test('refreshes the active provider from the upstream model list', async () => {
      const originalFetch = globalThis.fetch
      globalThis.fetch = mock(async () => {
        return new Response(JSON.stringify({
          data: [
            { id: 'glm-5.2', maxContextWindow: 1000, supportThinking: true },
            { id: 'minimax-m3', maxContextWindow: 512, supportThinking: true, supportImage: true },
          ],
        }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
      ...
      try {
        ...
        const provider = await svc.addProvider(sampleInput({
          ...
          models: {
            main: 'disabled-model',
            haiku: 'disabled-model',
            sonnet: 'disabled-model',
            opus: 'disabled-model',
          },
        }))
        await svc.activateProvider(provider.id)

        const refreshed = await svc.refreshActiveProviderModels()
        ...
        expect(refreshed?.availableModels).toEqual([
          { id: 'glm-5.2', contextWindow: 1_000_000, capabilities: ['reasoning'] },
          { id: 'minimax-m3', contextWindow: 512_000, capabilities: ['reasoning', 'image'] },
        ])
