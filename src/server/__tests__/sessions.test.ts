...
describe('SessionService', () => {
  ...
  it('should expand tilde workDir to the user home directory', async () => {
    const expectedHome = await fs.realpath(os.homedir())
    const { sessionId, workDir } = await service.createSession('~')
    const filePath = path.join(
      tmpDir,
      'projects',
      sanitizePath(expectedHome),
      `${sessionId}.jsonl`,
    )

    expect(path.resolve(workDir)).toBe(path.resolve(expectedHome))
    const stat = await fs.stat(filePath)
    expect(stat.isFile()).toBe(true)
  })
